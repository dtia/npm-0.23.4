...
NPMLocation.prototype = {
  ...
  processPackageConfig: function(pjson) {
    ...
    for (var d in pjson.dependencies)
      // 3/8/16: JSPM doesn't support git+ssh dependencies so this is a temporary fix
      // to make it the format it wants i.e. 
      // changing git+ssh://git@github.com:Skycatch/sky-lib.utils.git 
      // to       git://github.com/Skycatch/sky-lib.utils.git
      if (pjson.dependencies[d].match(/^git\+ssh[:+]/) && pjson.dependencies[d].indexOf(':') > 0)
        pjson.dependencies[d] = pjson.dependencies[d].replace('git+ssh://git@github.com:', 'git://github.com/');

      else if (!pjson.dependencies[d].match(/^(https?|git):/) && pjson.dependencies[d].indexOf(':') > 0)
